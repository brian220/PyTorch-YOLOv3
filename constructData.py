...
train_folder = "D:/user/Desktop/DLHW3/train/"
...
def construct_train_data(mat_file):
    f = h5py.File(mat_file,'r')
    ...
    for j in range(2000):
        ...
        im = Image.open(train_folder + img_name)
        img_width, img_height = im.size
        print(img_width, img_height)

        objectNum = len(row_dict['label'])
        for i in range(objectNum):
            label = int(row_dict['label'][i]) % 10
            left = row_dict['left'][i]
            top = row_dict['top'][i]
            bbox_width = row_dict['width'][i]
            bbox_height = row_dict['height'][i]

            x_center = (left + (bbox_width / 2)) * 1.0 / img_width
            y_center = (top + (bbox_height / 2)) * 1.0 / img_height
            width = bbox_width * 1.0 / img_width
            height = bbox_height * 1.0 / img_height
            
            print(label, x_center, y_center, width, height)

            fp = open("D:/user/Desktop/DLHW3/trainLabel/" + str(j + 1) + ".txt", "a")
            fp.write(str(label) + " " + str(x_center) + " " + str(y_center) + " " + str(width) + " " + str(height) + "\n")
            fp.close()

def contstruct_train_list_file():
    fp = open("D:/user/Desktop/DLHW3/train.txt", "a")
    f = h5py.File("D:/user/Desktop/DLHW3/train/digitStruct.mat",'r')
    for j in range(1200):
